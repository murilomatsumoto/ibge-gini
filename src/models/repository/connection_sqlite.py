...
class ManipuladorDB:
    ...
    def inserir_brasil(self, indice_gini):
        try:
            ...
            existing_entry = (
                session.query(Brasil).filter_by(Indice_Gini_1991=indice_gini).first()
            )

            if existing_entry:
                log.log_message(
                    "O índice Gini já está presente na tabela Brasil. Não foi realizada nenhuma inserção."
                )
            ...
            log.log_message(
                f"Erro de integridade ao inserir dados na tabela Brasil: {str(e)}"
            )
            session.rollback()
            return None
    ...
    def adicionar_estado(self, nome_estado, indice_gini, id_brasil):
        try:
            ...
            existing_state = (
                session.query(Estado).filter_by(Nome_estado=nome_estado).first()
            )

            if existing_state:
                log.log_message(
                    f"O estado {nome_estado} já está presente na tabela Estado. Não foi realizada nenhuma inserção."
                )
                ...
                if brasil:
                    estado_entry = Estado(
                        ...
                        Indice_Gini_1991=indice_gini,
                        ID_brasil=id_brasil,
                    )
                    session.add(estado_entry)
                    session.commit()
                    log.log_message(
                        f"Dados do estado {nome_estado} inseridos na tabela Estado com sucesso!"
                    )
                    return estado_entry.ID_estado
                else:
                    log.log_message(
                        f"Não foi possível encontrar o Brasil com o ID {id_brasil}. Nenhuma inserção foi realizada."
                    )
                    return None

        except IntegrityError as e:
            log.log_message(
                f"Erro de integridade ao inserir dados na tabela Estado: {str(e)}"
            )
            session.rollback()
            return None
        except Exception as e:
    ...
    def adicionar_cidade(self, nome_cidade, indice_gini, id_estado):
        try:
            ...
            existing_city = (
                session.query(Cidade).filter_by(Nome_cidade=nome_cidade).first()
            )

            if existing_city:
                log.log_message(
                    f"A cidade {nome_cidade} já está presente na tabela Cidade. Não foi realizada nenhuma inserção."
                )
                ...
                if estado:
                    ...
                    session.commit()
                    log.log_message(
                        f"Dados da cidade {nome_cidade} inseridos na tabela Cidade com sucesso!"
                    )
                    return cidade_entry.ID_cidade
                else:
                    log.log_message(
                        f"Não foi possível encontrar o estado com o ID {id_estado}. Nenhuma inserção foi realizada."
                    )
                    return None

        except IntegrityError as e:
            log.log_message(
                f"Erro de integridade ao inserir dados na tabela Cidade: {str(e)}"
            )
            session.rollback()
            return None
        except Exception as e:
