...
class ManipuladorDB:
    ...
    def inserir_brasil(self, indice_gini):
        try:
            ...
            if existing_entry:
                print("O índice Gini já está presente na tabela Brasil. Não foi realizada nenhuma inserção.")
                return existing_entry.ID_brasil
            else:
                brasil_entry = Brasil(Indice_Gini_1991=indice_gini)
                session.add(brasil_entry)
                session.commit()
                print("Dados inseridos na tabela Brasil com sucesso!")
                return brasil_entry.ID_brasil

        except IntegrityError as e:
            print(f"Erro de integridade ao inserir dados na tabela Brasil: {str(e)}")
            session.rollback()
            return None
        except Exception as e:
            print(f"Erro ao inserir dados na tabela Brasil: {str(e)}")
            return None
        finally:
            session.close()
    ...
    def adicionar_estado(self, nome_estado, indice_gini, id_brasil):
        try:
            ...
            if existing_state:
                print(f"O estado {nome_estado} já está presente na tabela Estado. Não foi realizada nenhuma inserção.")
                ...
                if brasil:
                    ...
                    session.commit()
                    print(f"Dados do estado {nome_estado} inseridos na tabela Estado com sucesso!")
                    return estado_entry.ID_estado
                else:
                    print(f"Não foi possível encontrar o Brasil com o ID {id_brasil}. Nenhuma inserção foi realizada.")
                    return None

        except IntegrityError as e:
            print(f"Erro de integridade ao inserir dados na tabela Estado: {str(e)}")
            session.rollback()
            return None
        except Exception as e:
            print(f"Erro ao inserir dados na tabela Estado: {str(e)}")
            return None
        finally:
            session.close()
    ...
    def adicionar_cidade(self, nome_cidade, indice_gini, id_estado):
        try:
            ...
            if existing_city:
                print(f"A cidade {nome_cidade} já está presente na tabela Cidade. Não foi realizada nenhuma inserção.")
                ...
                if estado:
                    ...
                    session.commit()
                    print(f"Dados da cidade {nome_cidade} inseridos na tabela Cidade com sucesso!")
                    return cidade_entry.ID_cidade
                else:
                    print(f"Não foi possível encontrar o estado com o ID {id_estado}. Nenhuma inserção foi realizada.")
                    return None

        except IntegrityError as e:
            print(f"Erro de integridade ao inserir dados na tabela Cidade: {str(e)}")
            session.rollback()
            return None
        except Exception as e:
            print(f"Erro ao inserir dados na tabela Cidade: {str(e)}")
            return None
        finally:
            session.close()
